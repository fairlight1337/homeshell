/**
 * @file Sha256sumCommand.hpp
 * @brief Compute SHA-256 checksums
 *
 * This command computes and verifies SHA-256 (256-bit) checksums,
 * similar to the Unix `sha256sum` command.
 *
...
class Sha256sumCommand : public ICommand
{
public:
    ...
    {
        return "Compute SHA-256 checksums";
    }
    ...
    Status execute(const CommandContext& context) override
    {
        ...
        for (const auto& filename : files)
        {
            ...
            else
            {
                ...
                if (hash.empty())
                {
                    std::cerr << "sha256sum: " << filename << ": No such file or directory\n";
                    continue;
                }
...
private:
    void showHelp() const
    {
        std::cout << "Usage: sha256sum [OPTION]... [FILE]...\n\n"
    ...
    bool verifyChecksums(const std::string& checksum_file) const
    {
        ...
        if (!file)
        {
            std::cerr << "sha256sum: " << checksum_file << ": No such file or directory\n";
            return false;
        }
